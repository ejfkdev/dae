...
**控制流已结构化**：支配树找出自然循环（回边 = 头支配尾），再按区域递归发射——两分支汇合
的写成 `if/else`，循环头写成 `while`，跳出循环的分支写成 `break`/`continue`。约 60% 的函数
完全结构化；其余保留 `goto` 并在函数头打 `NOTE` 标记，读者能一眼看出哪些文件是伪代码而非 Dart。

**还没做的**：表达式嵌套（`mem(qword ptr [FP + 8])` 还是平铺的，没有合成字段读取）、类型恢复。
认不出的指令原样输出为 `// unmapped:`，不做近似。

每次改动都有形态门禁（`tests/decompiler_shape.rs`）：每个产物文件花括号必须配平（不配平=静默
丢了分支）、函数体内语句必须正常结束、结构化率有下限。
